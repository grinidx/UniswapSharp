using AwesomeAssertions;
using UniswapSharp.Tamperproof;
using UniswapSharp.Tamperproof.Utils;

namespace UniswapSharp.Testing.Tamperproof;

// Ported from sdks/tamperproof-transactions/src/sign.test.ts. Keys/signatures copied verbatim.
public class SignTests
{
    private const string Data = "test data";

    private const string PrivateKeyEcdsaHex =
        "";

    private const string PrivateKeyEcdsa384Hex =
        "";

    private const string PrivateKeyEcdsa521Hex =
        "";

    private const string PrivateKeyEd25519Hex =
        "302e020100300506032b657004220420d4ee72dbf913584ad5b6d8f1f769f8ad3afe7c28cbf1d4fbe097a88f44755842";

    private const string PrivateKeyRsaHex =
        "";

    private const string PrivateKeyRsaPssHex = PrivateKeyRsaHex;

    [Fact]
    public void RS256_ReturnsExpectedSignature()
    {
        Signer.Sign(Data, PrivateKeyRsaHex, "RS256").Should().Be(
            "");
    }

    [Fact]
    public void RS384_ReturnsExpectedSignature()
    {
        Signer.Sign(Data, PrivateKeyRsaHex, "RS384").Should().Be(
            "");
    }

    [Fact]
    public void RS512_ReturnsExpectedSignature()
    {
        Signer.Sign(Data, PrivateKeyRsaHex, "RS512").Should().Be(
            "");
    }

    [Fact]
    public void EdDSA_ReturnsExpectedSignature()
    {
        Signer.Sign(Data, PrivateKeyEd25519Hex, "EdDSA").Should().Be(
            "0x0804a2a72f52d7afdaf18b78e1a48891a729be1bdde2b30366fd00a128bc37243aa75c36e8b0a93b71fe7dfd7b67bee0838e25acd26b8a81ad7074ae38f84102");
    }

    [Theory]
    [InlineData("PS256")]
    [InlineData("PS384")]
    [InlineData("PS512")]
    public void PSxxx_Is512ByteString(string alg)
    {
        string result = Signer.Sign(Data, PrivateKeyRsaPssHex, alg);
        result.Should().StartWith("0x");
        result[2..].Should().HaveLength(512);
    }

    [Theory]
    [InlineData("PS256")]
    [InlineData("PS384")]
    [InlineData("PS512")]
    public void PSxxx_IsNonDeterministic(string alg)
    {
        Signer.Sign(Data, PrivateKeyRsaPssHex, alg)
            .Should().NotBe(Signer.Sign(Data, PrivateKeyRsaPssHex, alg));
    }

    [Fact]
    public void ES256_Is128ByteString()
    {
        string result = Signer.Sign(Data, PrivateKeyEcdsaHex, "ES256");
        result.Should().StartWith("0x");
        result[2..].Should().HaveLength(128);
    }

    [Fact]
    public void ES256_IsNonDeterministic()
    {
        Signer.Sign(Data, PrivateKeyEcdsaHex, "ES256")
            .Should().NotBe(Signer.Sign(Data, PrivateKeyEcdsaHex, "ES256"));
    }

    [Fact]
    public void ES384_Is192ByteString()
    {
        string result = Signer.Sign(Data, PrivateKeyEcdsa384Hex, "ES384");
        result.Should().StartWith("0x");
        result[2..].Should().HaveLength(192);
    }

    [Fact]
    public void ES384_IsNonDeterministic()
    {
        Signer.Sign(Data, PrivateKeyEcdsa384Hex, "ES384")
            .Should().NotBe(Signer.Sign(Data, PrivateKeyEcdsa384Hex, "ES384"));
    }

    [Fact]
    public void ES512_Is264ByteString()
    {
        string result = Signer.Sign(Data, PrivateKeyEcdsa521Hex, "ES512");
        result.Should().StartWith("0x");
        result[2..].Should().HaveLength(264);
    }

    [Fact]
    public void ES512_IsNonDeterministic()
    {
        Signer.Sign(Data, PrivateKeyEcdsa521Hex, "ES512")
            .Should().NotBe(Signer.Sign(Data, PrivateKeyEcdsa521Hex, "ES512"));
    }

    [Fact]
    public void ObjectPayload_SignsUsingCanonicalJson()
    {
        var payload = new Dictionary<string, object?>
        {
            ["method"] = "eth_call",
            ["params"] = new Dictionary<string, object?>
            {
                ["b"] = 2,
                ["a"] = 1,
                ["c"] = CanonicalJson.Undefined,
            },
        };
        string canonical = CanonicalJson.CanonicalStringify(payload);

        string sigObj = Signer.Sign(payload, PrivateKeyRsaHex, "RS256");
        string sigStr = Signer.Sign(canonical, PrivateKeyRsaHex, "RS256");

        sigObj.Should().Be(sigStr);
    }

    [Fact]
    public void UnsupportedAlgorithm_Throws()
    {
        Action act = () => Signer.Sign(Data, PrivateKeyRsaHex, "INVALID_ALGO");
        act.Should().Throw<Exception>();
    }

    [Fact]
    public void KeyAlgorithmMismatch_Throws()
    {
        Action act = () => Signer.Sign(Data, PrivateKeyEcdsaHex, "PS256");
        act.Should().Throw<Exception>();
    }
}
