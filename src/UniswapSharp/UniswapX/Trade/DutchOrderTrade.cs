...
/// <summary>Port of uniswapx-sdk <c>trade/DutchOrderTrade.ts</c>.</summary>
public sealed class DutchOrderTrade<TInput, TOutput>
    where TInput : BaseCurrency
    where TOutput : BaseCurrency
{
    public TradeType TradeType { get; }
    public DutchOrder Order { get; }
    ...
    public DutchOrderTrade(TInput currencyIn, IReadOnlyList<TOutput> currenciesOut, DutchOrderInfo orderInfo, TradeType tradeType)
    {
        _currencyIn = currencyIn;
        _currenciesOut = currenciesOut;
        TradeType = tradeType;
        Order = new DutchOrder(orderInfo, currencyIn.ChainId);
    }
    ...
    public IReadOnlyList<CurrencyAmount<TOutput>> OutputAmounts =>
        ...
        {
            var currencyOut = FindCurrencyOut(output.Token);
            return CurrencyAmount<TOutput>.FromRawAmount(currencyOut, output.StartAmount);
        }).ToList();
